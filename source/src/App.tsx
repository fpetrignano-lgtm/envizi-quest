...
export default function Home(){
  ...
  if(screen==="questIntro"&&profile)return <main className="questIntroScreen"><header className="missionNav missionNavTrust"><button className="brand brandButton" onClick={reset}><span className="brandMark">e·</span><span>Envizi<br/>Impact Quest</span></button><div className="missionProgress"><span className="activeDot"/> IL QUEST</div>{renderTrustBar()}<button className="langMini" onClick={()=>setLanguage(language==="it"?"en":"it")}>{language==="it"?"EN":"IT"}</button></header><section className="questIntroBody"><div className="questIntroLeft"><img src={`./characters/${profile}-neutral.png`} className="questIntroProfileImg" alt={name}/><div className="questIntroProfileTag"><span className="statusDot"/><div><small>ESG MANAGER</small><strong>{name}</strong></div></div></div><div className="questIntroRight"><p className="eyebrow">{t.questIntroKicker}</p><p className="questIntroText">{t.questIntroText}</p><button className="actionButton questIntroCta" onClick={()=>setScreen("approachIntro")}>{t.questIntroCta}<b>→</b></button></div></section></main>;
